...
| conversationId / messageId / messageStreamId / requestId 防串话 | 每个会话、消息、流式输出、run、trace 必须有明确识别边界。               | 多轮追问、Trace、ToolCall、ModelCall       | Analysis, Observability                                  | 是                 | `conversationId`、`messageId`、`messageStreamId`、`runId`、`toolCallId`、`modelCallId`、`requestId` 边界 |
