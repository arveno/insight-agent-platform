...
- `ContentSection` 只负责 section 语义、eyebrow、title、extra 和 children 基础排列。
...
- `ContentSection` 通过 `layout="wrap" | "vertical"` 控制 children 的基础排列；同类卡片应直接作为 children 传入，而不是再包一层 dashboard/report/evidence 专用 panel。
- section 内按钮排列直接使用 Ant `Flex / Space`，不要额外抽 `ActionGroup`。
- `Dashboard` 这类 overview page 的标准结构固定为 `SectionStack -> Hero -> ContentSection(layout) -> Cards`。
- 业务模块需要特殊卡片时，只在 module 内轻封单张卡片，不额外创建 `Panel / Grid / ActionGroup` 分组层。
...
| `ContentSection`          | Shared Pattern     | `shared/layout/sections/ContentSection.tsx`        | Ant `Space / Typography / Flex`        | section eyebrow、title、extra、layout、children 基础排列 | 业务判断、排序分组、专用 panel 分支                            | `app`、`modules`                                     | `app`、`modules`        |
