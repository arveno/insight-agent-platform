...
- `ContentSection` 只负责 section header 和 children slot，不负责卡片区 `Row / Col`、`wrap`、`grid` 或 section 级响应式排列。
- section 内同类卡片应由调用方在 children 中直接使用 Ant `Row / Col` 排列：手机浏览器默认 `xs={24}` 单列，桌面根据内容使用 `lg={12}` 双列或 `lg={8}` 三列。
...
- `Dashboard`、`Metrics`、`Platform Operations` 这类 overview page 的标准结构固定为 `SectionStack -> Hero(optional) -> ContentSection -> Row / Col -> Cards`。
